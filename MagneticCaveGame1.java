...
public class MagneticCaveGame {
    ...
    private boolean hasWon(int row, int col) {
        ...
        for (int c = 0; c < BOARD_SIZE; c++) {
            if (board[row][c] == currentPlayer.getBrick()) {
                count++;
                if (count == 5) {
                    return true;
                }
        ...
        for (int r = 0; r < BOARD_SIZE; r++) {
            if (board[r][col] == currentPlayer.getBrick()) {
                count++;
                if (count == 5) {
                    return true;
                }
        ...
        count = 0; // r=5 , c=6
        int startRow = row - Math.min(row, col);//5-5=0
        int startCol = col - Math.min(row, col);//1
        ...
        startCol = col + Math.min(row, BOARD_SIZE - col - 1);
    ...
    private int evaluateDiagonals() {
        ...
        for (int row = 0; row <= BOARD_SIZE - 5; row++) {
            for (int col = 0; col <= BOARD_SIZE - 5; col++) {
                ...
                score += evaluateLine(row + 4, col, -1, 1);
            }
        }
        return score;
    }
